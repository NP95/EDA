...
GateDatabase::GateDatabase(const std::string& file_name) {
    // Removed DEBUG
    // cout << "[GateDatabase] Attempting to open: " << file_name << endl;
    
    // Removed DEBUG
    /*
    try {
        bool exists = std::filesystem::exists(file_name);
        cout << "[GateDatabase] std::filesystem::exists check: " << (exists ? "true" : "false") << endl; 
    } catch (const std::filesystem::filesystem_error& e) {
        cout << "[GateDatabase] Filesystem error checking existence: " << e.what() << endl; 
    }
    */
    ...
    // Removed DEBUG
    // cout << "[GateDatabase] File opened successfully with ifstream." << endl; 
...
void GateDatabase::insert(const std::string& gateName, GateInfo* gateInfo) {
    // Store keys in uppercase consistently
    std::string key = gateName;
    transform(key.begin(), key.end(), key.begin(), ::toupper);
    gate_info_lut_[key] = gateInfo;
}

// Find gate info using case-insensitive comparison (const correct)
const GateInfo* GateDatabase::get_gate_info(const std::string& gateName) const {
    // Since keys are stored uppercase, convert input to uppercase for lookup
    std::string upperGateName = gateName;
    std::transform(upperGateName.begin(), upperGateName.end(), upperGateName.begin(), ::toupper);
    
    auto it = gate_info_lut_.find(upperGateName);
    if (it != gate_info_lut_.end()) {
        return it->second;
    } else {
        // Gate not found
        // std::cout << "Gate not found: " << gateName << " (lookup: " << upperGateName << ")\n";
        return nullptr;
    }
}
