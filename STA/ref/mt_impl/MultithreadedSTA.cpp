...
#include "threadpool.hpp" // Include threadpool header
...
struct cktGates {
    ...
    int level = -1; // For tracking topological levels
};
...
// Modified topological sort to include level information
void topologicalSortWithLevels(const unordered_map<int, cktGates> &netlist, vector<int> &sortedList, unordered_map<int, cktGates> &mutableNetlist) {
    Debug::detail("Starting topological sort with level tracking...");
    ...
    queue<int> q;
    // First level (level 0) - nodes with no inputs
    for(const auto &[nodeID, node] : netlist) {
        if(inDegree[nodeID] == 0) {
            q.push(nodeID);
            mutableNetlist[nodeID].level = 0;
            Debug::trace("Adding zero-indegree node to queue: " + std::to_string(nodeID) + " (level 0)");
        }
    }
    ...
    while(!q.empty()) {
        ...
        // If this node is from a new level, process the previous level's nodes
        if (mutableNetlist[node].level > currentLevel) {
            // Process all nodes at the current level
            for (int levelNode : currentLevelNodes) {
                sortedList.push_back(levelNode);
                Debug::trace("Adding node to sorted list: " + std::to_string(levelNode) + " (level " + 
                             std::to_string(currentLevel) + ")");
            }
            currentLevelNodes.clear();
            currentLevel = mutableNetlist[node].level;
        }
        
        // Add current node to the current level's nodes
        currentLevelNodes.push_back(node);
        
        for(int fanOut : netlist.at(node).fanOutList) {
            if(--inDegree[fanOut] == 0) {
                // This fanout node's level is one more than the current node
                mutableNetlist[fanOut].level = mutableNetlist[node].level + 1;
                q.push(fanOut);
                Debug::trace("  Fanout " + std::to_string(fanOut) + " now has zero indegree, adding to queue (level " + 
                            std::to_string(mutableNetlist[fanOut].level) + ")");
            }
        }
    }
    
    // Process any remaining nodes in the current level
    for (int levelNode : currentLevelNodes) {
        sortedList.push_back(levelNode);
        Debug::trace("Adding node to sorted list: " + std::to_string(levelNode) + " (level " + 
                    std::to_string(currentLevel) + ")");
    ...
    Debug::info("Topological sort complete. Sorted " + std::to_string(sortedList.size()) + " nodes into " + 
               std::to_string(currentLevel + 1) + " levels.");
}
...
// Process a single node in the forward traversal (for parallel execution)
void processNodeForward(int node, unordered_map<int, cktGates> &netlist, 
                        const unordered_map<string, gate> &gates, std::atomic<double> &circuitDelay) {
    ...
    if (currentNode.nodeType == "INPUT" && !currentNode.isPrimaryInput) {
        ...
        double current = circuitDelay.load();
        double desired = std::max(current, currentNode.arrivalTime);
        // Keep trying to update the circuit delay with the maximum value
        while (desired > current && !circuitDelay.compare_exchange_weak(current, desired)) {
            desired = std::max(current, currentNode.arrivalTime);
        }
        return;
    }
    ...
    if (currentNode.isPrimaryOutput) {
        ...
        if (desired > current) {
            Debug::info("  Updated circuit delay to " + std::to_string(desired) + 
...
double parallelForwardTraversal(const vector<int> &sortedList, unordered_map<int, cktGates> &netlist, 
                                const unordered_map<string, gate> &gates, ThreadPool &threadPool) {
    Debug::info("Starting parallel forward traversal...");
    std::atomic<double> circuitDelay(0.0);
    ...
    for (int node : sortedList) {
        int level = netlist[node].level;
        levelToNodes[level].push_back(node);
        maxLevel = std::max(maxLevel, level);
    }
    
    Debug::info("Grouped nodes into " + std::to_string(maxLevel + 1) + " levels for parallel processing");
    
    // Process each level in sequence, but nodes at the same level in parallel
    for (int level = 0; level <= maxLevel; level++) {
        Debug::detail("Processing level " + std::to_string(level) + " with " + 
                    std::to_string(levelToNodes[level].size()) + " nodes");
        
        vector<std::future<void>> futures;
        
        // Process all nodes at this level in parallel
        for (int node : levelToNodes[level]) {
            // Submit task to thread pool
            futures.push_back(threadPool.enqueue([node, &netlist, &gates, &circuitDelay]() {
                processNodeForward(node, netlist, gates, circuitDelay);
            }));
        }
        ...
        Debug::detail("Completed processing level " + std::to_string(level));
    }
    
    Debug::info("Parallel forward traversal complete. Circuit Delay = " + std::to_string(circuitDelay) + " ps");
    return circuitDelay;
}

// Process a single node in the backward traversal (for parallel execution)
void processNodeBackward(int node, unordered_map<int, cktGates> &netlist, 
                       const unordered_map<string, gate> &gates, double requiredTimeVal) {
    cktGates& currentNode = netlist[node];
    std::string nodeStr = std::to_string(node);
...
// Process all nodes at a given level in the backward traversal in parallel
void parallelBackwardTraversal(const vector<int>& sortedList, unordered_map<int, cktGates> &netlist, 
                             const unordered_map<string, gate> &gates, double circuitDelay, ThreadPool &threadPool) {
    double requiredTimeVal = 1.1 * circuitDelay; 
    Debug::info("Starting parallel backward traversal with required time = " + std::to_string(requiredTimeVal) + " ps");
    ...
    // Group nodes by level (in reverse order)
    unordered_map<int, vector<int>> levelToNodes;
    int maxLevel = -1;
    ...
    // Process each level in reverse order, but nodes at the same level in parallel
    for (int level = maxLevel; level >= 0; level--) {
        ...
        for (int node : levelToNodes[level]) {
            // Submit task to thread pool
            futures.push_back(threadPool.enqueue([node, &netlist, &gates, requiredTimeVal]() {
                processNodeBackward(node, netlist, gates, requiredTimeVal);
            }));
        }
        ...
        Debug::detail("Completed processing level " + std::to_string(level) + " backwards");
    }
    
    Debug::info("Parallel backward traversal complete.");
}
...
int main(int argc, char *argv[]) {
    ...
    Debug::info("Starting Multithreaded Static Timing Analysis...");
    ...
    Debug::info("Initializing thread pool with " + std::to_string(numThreads) + " threads");
    ThreadPool threadPool(numThreads);
    
    // Parse input files timing
    ...
    Debug::info("Parsing completed in " + std::to_string(parseDuration.count()) + " ms");

    // Topological sort timing
    auto topoStartTime = std::chrono::high_resolution_clock::now();
    
    vector<int> sortedList;
    topologicalSortWithLevels(netlist, sortedList, netlist);
    
    auto topoEndTime = std::chrono::high_resolution_clock::now();
    auto topoDuration = std::chrono::duration_cast<std::chrono::milliseconds>(topoEndTime - topoStartTime);
    Debug::info("Topological sort completed in " + std::to_string(topoDuration.count()) + " ms");

    // Forward traversal timing
    auto forwardStartTime = std::chrono::high_resolution_clock::now();
    
    double circuitDelay = parallelForwardTraversal(sortedList, netlist, gates, threadPool);
    
    auto forwardEndTime = std::chrono::high_resolution_clock::now();
    auto forwardDuration = std::chrono::duration_cast<std::chrono::milliseconds>(forwardEndTime - forwardStartTime);
    Debug::info("Forward traversal completed in " + std::to_string(forwardDuration.count()) + " ms");
    
    // Backward traversal timing
    auto backwardStartTime = std::chrono::high_resolution_clock::now();
    
    parallelBackwardTraversal(sortedList, netlist, gates, circuitDelay, threadPool);
    
    auto backwardEndTime = std::chrono::high_resolution_clock::now();
    auto backwardDuration = std::chrono::duration_cast<std::chrono::milliseconds>(backwardEndTime - backwardStartTime);
    Debug::info("Backward traversal completed in " + std::to_string(backwardDuration.count()) + " ms");
    
    // Critical path identification timing
    auto critPathStartTime = std::chrono::high_resolution_clock::now();
    
    vector<int> criticalPath = CriticalPath(netlist);
    
    auto critPathEndTime = std::chrono::high_resolution_clock::now();
    auto critPathDuration = std::chrono::duration_cast<std::chrono::milliseconds>(critPathEndTime - critPathStartTime);
    Debug::info("Critical path identification completed in " + std::to_string(critPathDuration.count()) + " ms");
    ...
    Debug::info("Total runtime: " + std::to_string(totalDuration.count()) + " ms");
    Debug::info("Breakdown - Parse: " + std::to_string(parseDuration.count()) + 
               " ms, Topo: " + std::to_string(topoDuration.count()) +
               " ms, Forward: " + std::to_string(forwardDuration.count()) +
               " ms, Backward: " + std::to_string(backwardDuration.count()) +
               " ms, CritPath: " + std::to_string(critPathDuration.count()) + " ms");
    
    // Also print timing info to stdout
    cout << "Static Timing Analysis completed in " << totalDuration.count() << " ms" << endl;
    cout << "Circuit delay: " << circuitDelay << " ps" << endl;
